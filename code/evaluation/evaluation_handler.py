...
def create_model_info_string(alg_name, params):
    if (alg_name=='synverse')| (alg_name=='synverse_v2')|(alg_name=='synverse_v3')|(alg_name=='synverse_v4')|\
            (alg_name=='synverse_nogenex')|(alg_name=='decagon'):
        model_info_str = dict_to_str(params)
        model_param = params
...
def get_pos_neg_predictions_files(should_run_algs,  param_settings_dict):
    ...
    for alg in should_run_algs:
        param_settings_list = param_settings_dict[alg]  # all considered values for each param is here
...
def find_best_param(should_run_algs, param_settings_dict, dataset_split_params, config_map):
    # dataset_params, preprocess_params_str = get_data_preprocess_params(cross_val_type, config_map, kwargs)
    number_of_runs = config_map['runs']
    n_folds = config_map['split']['folds']
    ...
    for alg in should_run_algs:
        if alg in ['synverse','synverse_v2','synverse_v3','synverse_v4', 'synverse_tissuenet','synverse_nogenex','decagon','deepsynergy','dtf']:
            ...
            for run_ in range(number_of_runs):
                out_dir = config_map['project_dir'] + config_map['output_dir']['result'] \
                          + alg + '/' + dataset_split_params + '/run_'+str(run_)+'/'
                param_count=0
...
def plot_best_models(should_run_algs, param_settings_dict, dataset_split_params, kwargs, config_map):
    best_param_settings_dict = find_best_param(should_run_algs, param_settings_dict, dataset_split_params, config_map)
    #save best params
    ...
    for alg in should_run_algs:
        ...
        for run_ in range(number_of_runs):
            out_dir = config_map['project_dir'] + config_map['output_dir']['result']
            alg_spec_out_dir = alg + '/' + dataset_split_params
            alg_spec_out_dir_per_run = alg_spec_out_dir + '/' + 'run_' + str(run_) + '/'
            alg_spec_out_path_per_run = out_dir + alg_spec_out_dir_per_run
    ...
    #plots of best models from all algs
    plot_dir = config_map['project_dir'] + config_map['output_dir']['viz']+ 'all_alg/' + dataset_split_params +'/'

    os.makedirs(plot_dir, exist_ok=True)
    y_threshold = 1 / float(1 + neg_fact)
    eval_plot.plot_best_models_auprc( best_models_eval_score_dict,y_threshold, plot_dir)
    eval_plot.plot_best_models_auroc( best_models_eval_score_dict, plot_dir)
    eval_plot.plot_best_models_e_prec( best_models_eval_score_dict, early_prec_k, y_threshold, plot_dir)
