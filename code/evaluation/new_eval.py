...
def set_title_suffix(run_, model_param):
    # title_suffix = 'run_' + str(model_param['run_']) + '_pairs_' + str(model_param['min_pairs_per_cell_line']) + \
    #                '_' + str(model_param['max_pairs_per_cell_line']) + '_drugfeat_' + str(model_param['drug_feat_option']) + \
    #                '_e_' + str(model_param['epochs']) + '_lr_' + str(model_param['lr']) + '_batch_' + \
    #                str(model_param['batch_size']) + '_dr_' + str(model_param['dr'])
    title_suffix = 'run_' + str(run_)
...
def evaluate(should_run_algs,kwargs,  config_map):
    ...
    pos_neg_file_names = {alg: {} for alg in should_run_algs} #dictionary of dictionary of list of tuple. each tupe contain two strings\
                ...
                for lr in lrs:
                    for in_hid_dropout in in_hid_dropouts:
                        ...
                        model_param = {'min_pairs': min_pairs_per_cell_line,
                                       ...
                                       'h_sizes': layer_setup, 'lr': lr, 'e': epochs,
    ...
    best_models_eval_score = {alg: [] for alg in should_run_algs}
    for alg in should_run_algs:
        i = 0
        for pos_out_file, neg_out_file in pos_neg_file_names[alg]:
            ...
            for run_ in range(number_of_runs):
                ...
                title_suffix = set_title_suffix(run_, model_params[alg][i])
                ...
                e_prec_saving_file = out_dir + pos_out_file.split('.')[0].replace('pos_val_scores','') + '.csv'

                early_prec = compute.compute_early_prec(pos_neg_df, early_prec_k, e_prec_saving_file, force_run=False)
                _, _, _, _, auprc, auroc = compute.compute_roc_prc(pos_neg_df)
            ...
            if best_avg_auprc[alg] < avg_auprc:
                ...
                best_models_eval_score[alg] = eval_scores  #performance of the best model from each alg (at each run) is  kept here
...
# evaluate(['synverse', 'deepsynergy'], kwargs, config_map)

#
#
#
# def main(kwargs, config_map):
#     evaluate(['synverse', 'deepsynergy'], kwargs, config_map)
#
# if __name__ == '__main__':
#     config_map, kwargs = parse_args()
#     main(kwargs, config_map)