...
def run_SynVerse(inputs, params, **kwargs):
    ...
    # MARSY specific modification
    params.split_dir = params.split_dir + '/MARSY_data/'
    ...
    #sort synergy_df so that (a,b) and (b,a) edges appear as (max(a,b), min(a,b))
    ...
    synergy_df, dfeat_dict, cfeat_dict, drug_2_idx, cell_line_2_idx = load_filter_triplets_features(synergy_df,
                            drug_pids, cell_line_names, inputs, params, device, feat_filt=False, abundance_based_filt=False)
    ...
    for run_no in range(start_run, end_run):
        for split in params.splits:
            ...
            print(f'#common pairs in synergy_df: {len(rev_triplets)}')
            ...
            for (select_drug_feat, select_cell_feat) in drug_cell_feat_combs:
                ...
                run_manager = RunManagerFactory.get_run_manager(params, select_model_info, given_epochs, all_train_df,
                                                                train_idx, val_idx, select_dfeat_dict, select_cfeat_dict,
                                                                test_df, drug_2_idx, cell_line_2_idx, out_file_prefix,
                                                                '_val_true_', device, **kwargs)
