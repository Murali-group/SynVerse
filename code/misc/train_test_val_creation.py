...
# def create_drug_pair_folds_consider_pos_folds(synergy_df, number_of_folds, val_frac, prev_drug_pair_folds=None, common_pairs=None):
#
...
#     if prev_drug_pair_folds==None:
#         prev_drug_pair_folds = {i: {'test': [], 'train': [], 'val': [], 'val_es': []} for i in range(number_of_folds)}
#         common_pairs = []
#     else:
#         for fold in range(number_of_folds):
#             for split in ['train','test','val','val_es']:
#                 prev_drug_pair_folds[fold][split] = list(set(prev_drug_pair_folds[fold][split]).intersection(common_pairs))
#
#     drug_combs_rest = init_drug_combs.difference(common_pairs)
#     for fold in range(number_of_folds):
#         if n_drug_comb_in_test > len(prev_drug_pair_folds[fold]['test']):
#             drug_pair_folds[fold]['test'] = random.sample(drug_combs_rest,
#                                             n_drug_comb_in_test - len(prev_drug_pair_folds[fold]['test']))
#         drug_combs_rest = drug_combs_rest.difference(drug_pair_folds[fold]['test'])
#
#     drug_combs_rest = init_drug_combs.difference(common_pairs)
#     for fold in range(number_of_folds):
#         drug_combs_rest = drug_combs_rest.difference(drug_pair_folds[fold]['test'])
#         if n_drug_comb_in_val > len(prev_drug_pair_folds[fold]['val']):
#             drug_pair_folds[fold]['val'] = random.sample(drug_combs_rest, n_drug_comb_in_val - len(prev_drug_pair_folds[fold]['val']))
#
#         drug_combs_rest = drug_combs_rest.difference(drug_pair_folds[fold]['val'])
#
#         if n_drug_comb_in_val > len(prev_drug_pair_folds[fold]['val_es']):
#             drug_pair_folds[fold]['val_es'] = random.sample(drug_combs_rest, n_drug_comb_in_val-len(prev_drug_pair_folds[fold]['val_es']))
#
#         drug_combs_rest = drug_combs_rest.difference(drug_pair_folds[fold]['val_es'])
#         drug_pair_folds[fold]['train'] = list(copy.deepcopy(drug_combs_rest))
#
#         #now add drug_pairs from previouse drug_pair_folds:
#         for split in ['train','test','val','val_es']:
#              drug_pair_folds[fold][split] = list(drug_pair_folds[fold][split]) + list(prev_drug_pair_folds[fold][split])
#
#         drug_combs_rest = init_drug_combs.difference(common_pairs)
#
...
# def leave_comb_cell_line_cross_val(synergy_df, number_of_folds, val_frac, cell_lines_per_fold=None):
#     init_cell_lines = synergy_df['Cell_line'].unique()
#
#     n_cell_lines_in_test = int(len(init_cell_lines) / number_of_folds)
#     n_cell_lines_in_val = int(len(init_cell_lines) * val_frac)
...
#             for j in range(i+1, len(split_types)):
#                 df_1 = synergy_df[synergy_df.index.isin(folds[fold][split_types[i]])][['Drug1_pubchem_cid','Drug2_pubchem_cid']]
#                 df_2 = synergy_df[synergy_df.index.isin(folds[fold][split_types[j]])][['Drug1_pubchem_cid', 'Drug2_pubchem_cid']]
#
#                 pairs_1 = set(zip(df_1['Drug1_pubchem_cid'], df_1['Drug2_pubchem_cid']))
#                 pairs_2 = set(zip(df_2['Drug1_pubchem_cid'], df_2['Drug2_pubchem_cid']))
#
#                 pairs_1 = set([(max(x,y),min(x,y)) for x,y in pairs_1])
#                 pairs_2 = set([(max(x, y), min(x, y)) for x, y in pairs_2])
#
#                 common_btn_1_2  = len(pairs_1.intersection(pairs_2))
#                 print('fold_no: ', fold, len(pairs_1), len(pairs_2), common_btn_1_2)
#
#
#     return folds, cell_lines_per_fold

# def assert_leave_comb_cell_line(synergy_df, folds, num_folds):
#     split_types = ['train', 'test', 'val','val_es']
#     for fold in range(num_folds):
#         cell_lines = {x:set() for x in split_types}
#         drug_pairs = {x:set() for x in split_types}
#         for split in split_types:
#             df = synergy_df[synergy_df.index.isin(folds[fold][split])]
#             selected_triplets = set(zip(df['Drug1_pubchem_cid'], df['Drug2_pubchem_cid'], df['Cell_line']))
#             drug_pairs[split] = set([(x, y) for x, y, z in selected_triplets])
#             cell_lines[split] = set([z for x, y, z in selected_triplets])
#         for i in range(len(split_types)):
#             for j in range(i+1, len(split_types)):
#                 print(split_types[i], split_types[j])
#                 if len(drug_pairs[split_types[i]].intersection(drug_pairs[split_types[j]])) > 0:
#                     print(drug_pairs[split_types[i]].intersection(drug_pairs[split_types[j]]))
#                 # assert len(drug_pairs[split_types[i]].intersection(drug_pairs[split_types[j]]))==0,'problem in drug pairs'
#                 if len(cell_lines[split_types[i]].intersection(cell_lines[split_types[j]]))>0:
#                     print( cell_lines[split_types[i]].intersection(cell_lines[split_types[j]]))
#                 # assert len(cell_lines[split_types[i]].intersection(cell_lines[split_types[j]]))==0,'problem in cell lines'
...
def create_test_val_train_cross_val_folds(synergy_df, init_non_synergy_df, cross_val_type, number_of_folds, \
                                          neg_fact, val_frac, neg_sampling_type):
    '''
    ...
    if cross_val_type == 'leave_comb':
        ...
        #check that both pos and negative drug pairs in test split are unseen in training time as postive/neg samples.
        # for fold in range(number_of_folds):
        #     pos_neg_drug_pairs_train = drug_pair_folds[fold]['train'] + neg_drug_pair_folds[fold]['train']
        #     pos_neg_drug_pairs_test = drug_pair_folds[fold]['test'] + neg_drug_pair_folds[fold]['test']
        #     pos_neg_drug_pairs_val = drug_pair_folds[fold]['val'] + neg_drug_pair_folds[fold]['val']
        #     pos_neg_drug_pairs_val_es = drug_pair_folds[fold]['val_es'] + neg_drug_pair_folds[fold]['val_es']

            # assert len(set(pos_neg_drug_pairs_train).intersection(set(pos_neg_drug_pairs_val))) == 0, 'problem 1'
            # assert len(set(pos_neg_drug_pairs_train).intersection(set(pos_neg_drug_pairs_val_es))) == 0, 'problem 2'
            # assert len(set(pos_neg_drug_pairs_train).intersection(set(pos_neg_drug_pairs_test))) == 0, 'problem 3'
            # assert len(set(pos_neg_drug_pairs_val).intersection(set(pos_neg_drug_pairs_val_es))) == 0, 'problem 4'
            # assert len(set(pos_neg_drug_pairs_val).intersection(set(pos_neg_drug_pairs_test))) == 0, 'problem 5'
            # assert len(set(pos_neg_drug_pairs_val_es).intersection(set(pos_neg_drug_pairs_test))) == 0, 'problem 6'
