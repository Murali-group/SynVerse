...
class MLP_runner:
    def __init__(self,train_val_df, train_idx, val_idx, test_df, dfeat_dict, cfeat_dict, drug_2_idx,
        ...
        self.run_id = kwargs.get('run_id', self.bohb_params['run_id'])
        self.nic_name = kwargs.get('nic_name')
        self.shared_directory = kwargs.get('shared_dir')

        #hardcoded parameters for model training
        self.check_freq=2
        self.tolerance=10
    ...
    #get the used specified setting here about wandb and BOHB
        ...
        host = hpns.nic_name_to_host(self.nic_name)
        ...
        if self.worker:
            ...
            w = MLPWorker(self.train_val_dataset, self.undir_train_idx, self.undir_val_idx,
                ...
                sleep_interval=0.5, run_id=self.run_id, host=host)
            w.load_nameserver_credentials(working_directory=self.shared_directory)
        ...
        w = MLPWorker(self.train_val_dataset, self.undir_train_idx, self.undir_val_idx,
                input_dim=self.input_dim,n_folds=self.n_folds, check_freq=self.check_freq, tolerance=self.tolerance,
                is_wandb=self.is_wandb, device=self.device,
                sleep_interval=0.5, run_id=self.run_id, host=host, nameserver=ns_host, nameserver_port=ns_port)
        w.run(background=True)

        # Run an optimizer
        # We now have to specify the host, and the nameserver information
        bohb = BOHB(configspace=MLPWorker.get_configspace(),
                    run_id=self.run_id,
                    host=host,
                    nameserver=ns_host,
        ...
        res = bohb.run(n_iterations=n_iterations, min_n_workers=self.n_workers)
