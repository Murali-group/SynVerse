...
def save_val_eval (val_loss, layer_setup, lr, input_dropout, dropout, batch_size, \
                              epochs, act_func,use_genex, use_target, out_dir ):
    model_info = '_layers_' + str(layer_setup) + '_e_' + str(epochs) + '_lr_' + str(
        lr) + '_batch_' + str(batch_size) + '_dr_' + \
                   str(input_dropout) + '_' + str(dropout) + '_act_' + str(act_func) + '_use_genex_' + str(use_genex) + \
                   '_use_target_' + str(use_target)

    val_file = out_dir + 'val_loss'+model_info + '.txt'
...
def run_deepsynergy_model(drug_maccs_keys_targets_feature_df,drug_maccs_keys_feature_df,\
    ...
    lr = ds_params['learning_rate']
    epochs = ds_params['epochs']
    act_func = ds_params['act_func']
    batch_size = ds_params['batch_size']
    input_dropout = ds_params['in_hid_dropout'][0]
    dropout = ds_params['in_hid_dropout'][1]
    ...
    save_drug_syn_probability(test_pos_df, test_neg_df, layers, lr, input_dropout, dropout, batch_size, \
                              epochs, act_func,use_genex, use_target, out_dir)
    save_val_eval(val_loss,layers, lr, input_dropout, dropout, batch_size, \
                              epochs, act_func,use_genex, use_target, out_dir )
