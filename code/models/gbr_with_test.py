...
def save_val_eval(val_mse, model_info, use_genex, use_target, out_dir):

    val_file = out_dir + 'val_eval_'+model_info +'_use_genex_' + str(use_genex)+\
                   '_use_target_' +str(use_target)+ '.txt'
...
def run_gbr_model(drug_maccs_keys_targets_feature_df,drug_maccs_keys_feature_df,\
    ...
    for fold in range(number_of_folds):
        ...
        gbr = GradientBoostingRegressor(loss='ls', learning_rate=learning_rate,n_estimators=n_estimators,
                                        max_depth=max_depth, random_state=0)
        model = gbr.fit(train_feat, train_label)

        #prediction on val
        val_predictions = model.predict(val_feat)

        val_mse= metrics.mean_squared_error(val_label, val_predictions)
    ...
    model_info = 'est_'+ str(n_estimators)+'_depth_'+str(max_depth) + '_lr_'+str(learning_rate)
    save_test_drug_syn_probability(test_pos_df, test_neg_df, model_info, use_genex, use_target, out_dir)
    save_val_eval(val_mse, model_info, use_genex, use_target, out_dir)
