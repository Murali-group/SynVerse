...
from torch_geometric.nn import GCNConv, GAE, VGAE, SAGEConv, GMMConv, GATConv,RGCNConv
...
class EdgeTypeSpecGCNLayerLocal(torch.nn.Module):
    ...
    def forward(self, node_feat_dict, train_pos_edges_dict):
        ...
        for subtype in range(len(edge_index_list)):
            ...
            output += x1
        # print('GCN edge_type: ', self.edge_type)
        # print('GCN edge index size: ', int(edge_index_list[0].size()[1]))
...
class EdgeTypeSpecGCNLayerRel(torch.nn.Module):  # in this function I have used one weight matrix for all drug_drug cell_lines
    ...
    def forward(self, node_feat_dict, train_pos_edges_dict):
        ...
        subtype_idx = 0
        edge_type = []
        edge_idx_all = torch.LongTensor().to(dev)
        for edge_index in edge_index_list:
            ...
            edge_idx_all = torch.cat([edge_idx_all, edge_index],1)
            print('RGCN subtype index: ', subtype_idx)
            subtype_idx += 1

        print('RGCN edge_type: ',len(edge_type))
        print('RGCN edge index size: ', int(edge_idx_all.size()[1]))
        edge_type = torch.LongTensor(edge_type).to(dev)
        output = self.gcn(F.dropout(x.float(), p=self.dr, training=True), edge_idx_all, edge_type=edge_type)
...
class NNDecoder(torch.nn.Module):
    ...
    def __init__(self, edge_type, n_sub_types, w_dim, hidden_layer_setup):
    ...
    def forward(self, z, batch_edges, edge_sub_type_idx, sigmoid=True):
        ...
        if self.edge_type == 'drug_drug':
            ...
            concatenated_drug_drug_embedding =\
                torch.cat((drug1_embed, drug2_embed), axis=1).\
...
def run_synverse_nogenex_model(ppi_sparse_matrix, gene_node_2_idx, drug_target_df, drug_maccs_keys_feature_df,\
                       ...
                       config_map, use_drug_based_batch_end, save_best_model=False):
    ...
    for fold_no in range(number_of_folds):
        best_model_no = 0
        project_name = 'synverse_nogenex_' + machine_name
        with wandb.init(project=project_name, config=config_map):
            ...
            for edge_type in edge_types:
                ...
                if decoder_names[edge_type]=='bilinear':
                    ...
                    decoders[edge_type] = NNDecoder(edge_type,n_sub_types,h_sizes[-1]*2,nndecoder_h_size)
