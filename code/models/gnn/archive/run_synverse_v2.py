...
class EdgeTypeSpecGCNLayerLocal(torch.nn.Module):
    ...
    def forward(self, node_feat_dict, train_pos_edges_dict):
        ...
        for subtype in range(len(edge_index_list)):
            ...
            #implementing relu i.e. the activation function after summation from all the cell lines
            x1 = self.gcn[subtype](x1, edge_index_list[subtype]) #new as of 09/15/2021
            x1 = F.normalize(x1, dim=1, p=2)  #v2: p=2 means l2-normalization
            output += x1
        # Nure: look into this normalization
        output = F.relu(output) #new as of 09/15/2021
...
def run_synverse_model(ppi_sparse_matrix, gene_node_2_idx, drug_target_df, drug_maccs_keys_feature_df,
    ...
    for fold_no in range(number_of_folds):
        best_model_no = 0
        project_name = 'synverse_v2_' + machine_name
