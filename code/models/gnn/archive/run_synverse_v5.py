...
class EdgeTypeSpecGCNLayerLocal(torch.nn.Module):
    ...
    def forward(self, node_feat_dict, train_pos_edges_dict):
        ...
        for subtype in range(self.n_edge_subtype):
            ...
            if self.edge_type != 'gene_gene':
                ...
                x1 = F.relu(self.gcn[subtype](x1, edge_index_list[0])) #for gene_gene edges same network will be used for all cell lines
            output += x1
...
def run_synverse_model(ppi_sparse_matrix, gene_node_2_idx, drug_target_df, drug_maccs_keys_feature_df,
    ...
    for fold_no in range(number_of_folds):
        best_model_no = 0
        project_name = 'synverse_' + machine_name
        with wandb.init(project=project_name, config=config_map):
            ...
            #commented out at version 5
            # edge_type_wise_number_of_subtypes = {}
            # for edge_type in edge_types:
            #     edge_type_wise_number_of_subtypes[edge_type] = len(train_pos_edges_dict[edge_type])

            #added in version 5
            edge_type_wise_number_of_subtypes = {'gene_gene': total_cell_lines,'drug_drug': total_cell_lines,\
                                                 'drug_target': 1, 'target_drug': 1}
                ...
                if epoch % 2 == 0:
                    ...
                    if loss_dd_val_es >= min_loss_dd_val_es:
                        ...
                        if no_improvement_val_loss > patience:
                            ...
                            save_model_info_with_loss(best_model_dd_val_loss, encoder_type, dd_decoder_type, nndecoder_h_size, h_sizes,
                                                      use_drug_feat_option, lr,
